...
void Backend::setAppTitle(const std::string& vLabel) {
    if (!vLabel.empty()) {
        char bufTitle[1024];
        snprintf(bufTitle, 1023, "%s Server Beta %s - %s", WookieLinks_Label, WookieLinks_BuildId, vLabel.c_str());
        glfwSetWindowTitle(m_MainWindowPtr, bufTitle);
    } else {
        char bufTitle[1024];
        snprintf(bufTitle, 1023, "%s Server Beta %s", WookieLinks_Label, WookieLinks_BuildId);
        glfwSetWindowTitle(m_MainWindowPtr, bufTitle);
    }
}
